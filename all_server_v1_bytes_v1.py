...
class ConnectionManager:
    ...
    async def proxy_message(self, client: WebSocket, message: Any):
        """消息代理处理"""
        async with self.ws_client.connect() as client:
            # 根据消息类型发送数据
            if isinstance(message, str):
    ...
    try:
        async with manager.ws_client.connect() as client:
            ...
            while True:
                ...
                try:
                    # message = await websocket.receive_text()
                    message = await asyncio.wait_for(websocket.receive_bytes(), timeout=20.0)
                    # 设置超时，避免无限等待
                    logger.debug(f"Received message: {message[:200]}...")
                    # 消息处理管道
                    await client.proxy_message(client, message)
                except Exception as e:
